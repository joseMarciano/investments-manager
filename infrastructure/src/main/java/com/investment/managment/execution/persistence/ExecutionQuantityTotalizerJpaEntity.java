...
@Entity(name = "ExecutionQuantityTotalizer")
@Table(name = "EXECUTIONS_QUANTITY_TOTALIZER")
@NoArgsConstructor
@Getter
@Setter
@Builder(access = AccessLevel.PRIVATE)
@AllArgsConstructor(access = AccessLevel.PRIVATE)
public class ExecutionQuantityTotalizerJpaEntity {
    @Id
    @Column(name = "ID")
    private String id;
    ...
    private ExecutionQuantityTotalizerJpaEntity origin;
    ...
    public Execution toAggregate() {
        return Execution.with(
                ...
                ofNullable(this.origin).map(ExecutionQuantityTotalizerJpaEntity::getId).map(ExecutionID::from).orElse(null),
